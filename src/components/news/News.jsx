import { Swiper, SwiperSlide } from 'swiper/react';
import 'swiper/css';
import "./News.scss"
import 'swiper/css/navigation';
import { Navigation } from 'swiper/modules';
import { BsArrowRightShort } from "react-icons/bs"
import Aos from "aos"
import "aos/dist/aos.css"   
import teacher from "./teacher.jpg"

export function News() {
  return (
    <div data-aos="fade-up" data-aos-anchor-placement="top-bottom" className='container3'>
        <div className='news'>
        <div className='newsTitle'>
            <h2>Ustozlar</h2>
        </div>
        <Swiper
        breakpoints={{
            400: {
                slidesPerView:1,
                spaceBetween:10,
            },
            550: {
                slidesPerView:2,
                spaceBetween:10,
            },
            900:{
                slidesPerView:3,
                spaceBetween:10,
            },
            1280: {
                slidesPerView:4,
                spaceBetween:30,
            }
            }}
            className="mySwiper1"
            >
                <SwiperSlide className='mySwiperSections'>
                    <img src="https://www.venkateshwaragroup.in/vgiblog/wp-content/uploads/2022/09/Untitled-design-2-1.jpg" alt="" />
                    <div className='divTitles'>
                        <p style={{height:"100px"}}>Nasiba Rahmonovna<br/><br/> Maktab Direktor</p>
                        <div className='newsBilmadm'>
                            <p>Kirish</p>
                            <BsArrowRightShort style={{fontSize:"40px"}}/>
                        </div>
                    </div>
                </SwiperSlide>
                <SwiperSlide className='mySwiperSections'>
                    <img src="https://www.klassroom.com/cdn/shop/articles/howdoIbecomeateacherlaterinlife_691984306.jpg?v=1649211447" alt="" />
                    <div className='divTitles'>
                        <p style={{height:"100px"}}>Gulchehra Bobohonovna<br/><br/>Direktor o'rinbosari</p>
                        <div className='newsBilmadm'>
                            <p>Kirish</p>
                            <BsArrowRightShort style={{fontSize:"40px"}}/>
                        </div>
                    </div>
                </SwiperSlide>
                <SwiperSlide className='mySwiperSections'>
                    <img src="https://www.hindustantimes.com/ht-img/img/2023/09/02/1600x900/teachers_day_1693652054152_1693652065719.jpg" alt="" />
                    <div className='divTitles'>
                        <p style={{height:"100px"}}>Arabjonova Malika<br/><br/>Manaviy va Marifiy...</p>
                        <div className='newsBilmadm'>
                            <p>Kirish</p>
                            <BsArrowRightShort style={{fontSize:"40px"}}/>
                        </div>
                    </div>
                </SwiperSlide>
                <SwiperSlide className='mySwiperSections'>
                    <img src="data:image/jpeg;base64," alt="" />
                    <div className='divTitles'>
                        <p style={{height:"100px"}}>Nasridinova Shoira<br/><br/>Direktor o'rinbosari</p>
                        <div className='newsBilmadm'>
                            <p>Kirish</p>
                            <BsArrowRightShort style={{fontSize:"40px"}}/>
                        </div>
                    </div>
                </SwiperSlide>
                <SwiperSlide className='mySwiperSections'>
                    <img src="https://www.learningpotential.gov.au/sites/default/files/styles/large/public/2019-07/PS57-Meeting-teachers.jpg?itok=I1uESp_r" alt="" />
                    <div className='divTitles'>
                        <p style={{height:"100px"}}>Tajiddinova Shaxnoza<br/><br/>Manaviy va Marifiy...</p>
                        <div className='newsBilmadm'>
                            <p>Kirish</p>
                            <BsArrowRightShort style={{fontSize:"40px"}}/>
                        </div>
                    </div>
                </SwiperSlide>
            </Swiper>
        </div>
    </div>
  );
}
export default News

